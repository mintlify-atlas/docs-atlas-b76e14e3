...
`create_user_profile` inserts a row into `public.users` using the service-role client. It bypasses RLS and is used internally by `sign_up()`, but you can also call it directly if you need fine-grained control.
...
<ResponseField name="already_registered" type="bool">
  `True` when the user already has a `public.users` record. No new record is created and `current_user` reflects the existing profile.
</ResponseField>

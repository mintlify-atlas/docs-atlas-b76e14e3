...
The `Settings` class extends `pydantic_settings.BaseSettings`. Fields map directly to environment variable names.
...
When running Supabase locally using OrbStack, `SUPABASE_URL` is not read from the environment directly for internal routing. Instead, the `SUPABASE_API_URL` constant and database host are constructed automatically from `PROJECT_REF`:

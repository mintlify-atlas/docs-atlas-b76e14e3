...
|---|---|---|
| `SUPABASE_URL` | Auth only | Supabase project URL (e.g. `https://<ref>.supabase.co`) |
| `SUPABASE_ANON_KEY` | Auth only | Public/anonymous API key for client-side operations |
| `SUPABASE_SERVICE_ROLE_KEY` | Auth only | Service-role key used by the server-side client (bypasses RLS) |

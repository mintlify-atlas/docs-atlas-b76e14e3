...
When a reference cannot be parsed, the block reads `[ref] — could not parse reference`. When a valid reference is not found in the corpus, the block reads `[ref] — not found in corpus`. Returns `"No passages found."` if the `references` list is empty or all references fail.
